(() => {
  ...
  function enterSourceMode(mode) {
    ...
    show(els.cameraWrap, mode === "camera");
    if (mode === "camera") {
      startCamera();
    } else {
      stopCamera();
  ...
  function bindCropperEvents() {
    ...
    // Source buttons
    if (els.sourceUploadBtn) {
      els.sourceUploadBtn.addEventListener("click", () => enterSourceMode("upload"));
    }
    if (els.sourceCameraBtn) {
      els.sourceCameraBtn.addEventListener("click", () => enterSourceMode("camera"));
    }
    if (els.modalFileInput) {
